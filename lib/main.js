 ...
 *
 * - about:imap-raw, a less simple webpage that connects to an IMAP server using
 *    imap.js and gets a folder list and the first 10 starred/flagged messages.
 *    It dumps the JS representations of those out as HTML tables.
 *
 ...
 * - THE API OBJECTS WE EXPOSE ARE NOT SECURE / HARDENED.  I was trying to wrap
 *    them by using existing Jetpack mechanisms, but none of them line up with
 *    trying to largely reuse the XPCOM implementation and I haven't gotten
 *    around to securing them yet because the APIs are only exposed to code
 *    written in here.  DO NOT EXPOSE THE APIs TO OTHER CONTENT WITHOUT
...
function runSyncTest() {
  ...
  $tabBrowser.addTab("about:imap-client", {
    onLoad: function(event) {
      ...
      // there are obviously other ways to ensure that ordering is not a problem
      $timers.setTimeout(function quickhack() {
        ...
        win.console.log("trying to invoke API w/logfunc");
        // I can't pass functions through this barrier anymore.  Not sure
        // what's up.  Does't super matter, but it is annoying.
        win.wrappedJSObject.goSync(accountDef);
      }, 200);
    }
  });
}
