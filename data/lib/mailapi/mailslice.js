 ...
 * === Batching ===
 *
 * Headers are either notified one-by-one using onHeaderAdded/onHeaderModified/
 * onHeaderRemoved or we are told about them in an explicit batch using
 * batchAppendHeaders.  We only get explicit batches when loading directly from
 * the database; any time network traffic is involved things will be
 * one-by-one.
 *
 * We want to batch so that UI will inherently cluster its DOM manipulations and
 * thereby minimize the amount of reflowing and painting that occurs.
 *
 * Since this class is already reasonably complicated, we push the actual
 * batching into the `SliceBridgeProxy`.
 */
...
MailSlice.prototype = {
  ...
  setStatus: function(status, requested, moreExpected, flush,
                      progress) {
    if (!this._bridgeHandle)
      return;
  ...
  onHeaderAdded: function(header, syncDriven, messageIsNew) {
    ...
    //  amount.  Note that this does not guarantee that we won't end up with
    //  more headers than originally planned; if we get told about headers
    //  earlier than the last slot, we will insert them and grow without forcing
    //  a removal of something else to offset.
    if (hlen >= this.desiredHeaders && idx === hlen)
      return;
    // If we are inserting (not at the end), then be sure to grow the number of
    // desired headers to be consistent with the number of headers we have.
