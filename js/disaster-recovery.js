define(function(require) {
   ...
   * releasing the mutex, closing any socket (if applicable), and
   * logging an error as best we can.
   *
   * In order to do this, we must track the following:
   *
   * - Which account a socket is attached to (in case it throws)
   * - Whether or not an account is holding a mutex
  ...
  var accountToMutexMap = new WeakMap();
  ...
  var DisasterRecovery = {
    ...
    // Monitor in-progress mutex holders in case they must be released.

    setCurrentMutexCall: function(account, mutexDoneCallback) {
      accountToMutexMap.set(account, mutexDoneCallback);
    },

    clearCurrentMutexCall: function(account) {
      accountToMutexMap.delete(account);
    },
    ...
    handleDisastrousError: function(e, account) {
      var op, jobDoneCallback, mutexDoneCallback;
      if (account) {
        ...
        mutexDoneCallback = accountToMutexMap.get(account);
      }
      ...
      if (account) {
        ...
        if (mutexDoneCallback) {
          console.warn('Releasing mutex.');
          mutexDoneCallback();
          slog.log('disaster-recovery:released-mutex');
        } else {
          console.warn('No mutex was held.');
        }
      } else {
